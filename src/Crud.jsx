...
const ListEmployee = () => {
    ...
    const fetchData = async () => {
      try {
        ...
        const response = await fetch("http://localhost:5000/listemployee", {
          method: "GET",
          headers: {
            Authorization: `Bearer ${token}`, // Fixed syntax issue
  ...
  const viewEmployee = (employeeID) => {
    navigate(`/overviewemployee/${employeeID}`); // Fixed missing backticks
  };

  const totalEmployees = tableData.length;
  const activeEmployees = tableData.filter(
          ...
          {filteredData.map((row, index) => (
            <tr key={index}>
              ...
              <td>
                <FaCircleInfo
                  className="info-icon"
                  onClick={() => viewEmployee(row.EmployeeID)}
                />
              </td>
            </tr>
          ))}
